...
#include <Box2D/Box2D.h>
	*/
	...
	private native long jniCreateParticleSystem(long worldAddr, float radius, float pressureStrength, 
		...
		def.maxCount = maxCount; //This is A test modidfication
	...
	*/
	...
	public float[] getParticleWeightBuffer(){
		return jniGetWeightBuffer(addr);
	}
	...
	*/
	...
	private native float[] jniGetWeightBuffer(long addr); /*
		b2ParticleSystem* system = (b2ParticleSystem*)addr;
		int32 count =  system->GetParticleCount();
		...
		jfloat fill[count];
		for(int i = 0; i < count; i++) {
			fill[i] = system->GetWeightBuffer()[i];
		}
